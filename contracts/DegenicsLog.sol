...
contract DegenicsLog is Base {
    ...
    modifier onlySpecimenContract() {
        require(eternalStorage.getAddress(keccak256(abi.encodePacked("contract.address", "Specimen"))) == msg.sender, "Only registred contract" );
        _;
    }
    ...
    function addSpecimenLog(bytes32 number, string memory logType, string memory log) public onlySpecimenContract {
    ...
    function countSpecimenLog(bytes32 number) public onlySpecimenContract returns(uint){
        return eternalStorage.getUint(keccak256(abi.encodePacked( "Specimen.Log.count",number)));
    }

    function specimenLogByIndex(bytes32 number, uint _index) public onlySpecimenContract returns(uint index, 
